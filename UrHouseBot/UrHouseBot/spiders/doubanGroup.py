...
class DoubangroupSpider(scrapy.Spider):
    ...
    def parse_before_login(self, response):
        ...
        captcha_image_url = response.xpath(
            SELECTOR.userpwdField).extract_first()
        if captcha_image_url is None:
            ...
            formdata = {
                'ck': '',
                'name': private.doubanAccount,
                'password': private.doubanPwd,
                'remember': 'false',
                'ticket': ''
            }
    ...
    def startCrawl(self):
        ...
        slackMe('\n\n' +
        datetime.datetime.today().strftime('%Y-%m-%d %H:%M') + "=" * 50 + datetime.datetime.today().strftime('%Y-%m-%d %H:%M') +
            '\n\n')
        self.loadCrawled()
        url = URLS.groupsPage
        needCrawl = []
        for title, dic in self.titles.items():
            ...
            if dic.get("hadReq") and not dic.get("hadResp"):
                ...
                if self.isBlock(link, oTitle):
                    continue
                req = scrapy.Request(link, headers=self.headers, dont_filter=True, callback=self.parse_detail_article)
                req.meta["tag_info"] = f'上次未完成: 帖子: -> {oTitle}:{dic.get("link")}'
                req.meta["title"] = oTitle
                needCrawl.append(req)
        
        return [scrapy.Request(url, dont_filter=True, headers=self.headers, callback=self.parse)]+needCrawl
    ...
    def parseIfNeed(self, response):
        ...
        if (len(validLinks) < 15):
            ...
            self.logger.info(f'🚯🚯该小组已长期不活跃, 加进 dead 和 block: {t}, 首页帖子: {len(dates)} 个, url: {response.url}')
            self.saveCrawled()
            ...
            for index, link in enumerate(validLinks):
              ...
              request = scrapy.Request(link, headers=self.headers, callback=self.parse_detail_article)
        ...
        if (len(validLinks) > 40):
          request = scrapy.Request(l, headers=self.headers, dont_filter=True, callback=self.parseIfNeed)
    ...
    def parse_group(self, response):
        ...
        needMonitor = self.monitors.get(monitorKey, {}).get('monitor', False) #[monitorKey]['monitor']
        ...
        for idx, t in enumerate(titles):
            ...
            request.meta["tag_info"] = f'{tag_info}, 帖子: {idx}/{totalTitleNum} -> {colorTitle(title)}'
            request.meta["title"] = title
            self.markReqTitle(title, link)
            yield request
        ...
        if next and need_more and not needMonitor:
            ...
            self.logger.info(f'小组页_拿不到下一页或不需要更多: {next} / needMore: {need_more}, 是否监控: {needMonitor}')
            if monitorKey and self.monitors[monitorKey]:
                self.monitors[monitorKey]['monitor'] = True
    ...
    def parse_search_result(self, response):
        ...
        self.logger.info(f'[监控]: ({needMonitor})_组位置: {tag_info} 页面->{len(titles)}条title\n\n' + '^' * 100)
        totalTitleNum = len(titles)
        if (totalTitleNum == 0) or (page == 1 and totalTitleNum < 47):
            # 加入黑名单, 这个以后就直接过滤掉
            self.logger.info('条件: (totalTitleNum == 0) or (page == 1 and totalTitleNum < 47)')
            self.logger.info(
        ...
        for idx, t in enumerate(titles):
            ...
            request = scrapy.Request(link,
                                     headers=self.headers,
                                     callback=to_parse)
            request.meta["tag_info"] = f'{tag_info}, 帖子: {idx}/{totalTitleNum} -> {title}'
            request.meta["title"] = title
            request.meta["monitorKey"] = monitorKey
            self.markReqTitle(title, link)
            yield request
        ...
        if next and need_more and not needMonitor:
            ...
            self.logger.info(f'小组查询关键字_拿不到下一页或不需要更多: {next} / needMore: {need_more},  是否监控: {needMonitor}')
            if monitorKey and self.monitors[monitorKey]:
                self.monitors[monitorKey]['monitor'] = True
            link = self.monitors[monitorKey]['link']
            if self.isBlock(link):
    ...
    def parse_detail_article(self, response):
        ...
        # 获取楼主
        author = None
        if response.css('.from a::text') and response.css('.from a::text').extract_first():
